...
def main(cfg):
    ...
    # Get FAR and FRR at the best accuracy threshold
    ...
    print("Accuracy: {:.6f}".format(best_accuracy))
    print("FAR at best accuracy: {:.6f}".format(best_far))
    print("FRR at best accuracy: {:.6f}".format(best_frr))

    print("\nTP, FP, FN, TN at best threshold:")
    predictions = (scores <= best_threshold).astype(int)
    TP = np.sum((predictions == 1) & (labels == 1))
    FP = np.sum((predictions == 1) & (labels == 0))
    FN = np.sum((predictions == 0) & (labels == 1))
    TN = np.sum((predictions == 0) & (labels == 0))
    print("TP at best threshold: {:.6f}".format(TP))
    print("FP at best threshold: {:.6f}".format(FP))    
    print("FN at best threshold: {:.6f}".format(FN))
    print("TN at best threshold: {:.6f}".format(TN))

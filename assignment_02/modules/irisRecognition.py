...
class irisRecognition(object):
    def __init__(self, cfg):
        ...
        self.n_neighbours = cfg["n_neighbours"]
    ...
    # FRR: 0.139183
    # Accuracy: 0.849262
    def extractIBBCode(self, polar):
        if polar is None:
            return None
            
        # Convert polar to tensor
        polar = torch.tensor(polar, dtype=torch.int64)
        ...
        dy = (radius * torch.sin(theta)).round().long()
        dx = (radius * torch.cos(theta)).round().long()

        # Get all pixels that are 'valid'
        centers = polar[radius:-radius, radius:-radius].unsqueeze(-1)
        
        # Get neighbors for all valid pixels
        neighbors = torch.stack([
            polar[radius+dy[k]:polar.shape[0]-radius+dy[k], 
                  radius+dx[k]:polar.shape[1]-radius+dx[k]] 
            for k in range(n_points)
        ], dim=-1)

        # Compare each valid pixel with its neighbors
        comparisons = (neighbors >= centers).to(torch.int64)
        
        # Convert binary pattern to decimal
        powers = torch.pow(2, torch.arange(n_points-1, -1, -1))
        binary_codes = torch.sum(comparisons * powers, dim=-1)
        
        # Create output tensor of same size as input matrix
        codeBinaries = torch.zeros_like(polar, dtype=torch.float32)
        codeBinaries[radius:-radius, radius:-radius] = binary_codes
    ...
    def preprocess_image(self, image):
        """
        Preprocess the iris image to enhance features and reduce noise
        """
        ...
        # Small kernel (3x3) to preserve edge details
        image = cv2.GaussianBlur(image, ksize=(3,3), sigmaX=1)

        # 3. Enhance edges using unsharp masking
        # This makes texture patterns more pronounced
    ...
    def matchIBBCodes(self, codes1, codes2):
        ...
        codes1 = codes1.to(self.device)
        codes2 = codes2.to(self.device)
        ...
        sigma1_sq = codes1.var()
        sigma2_sq = codes2.var()
        sigma12 = torch.mean((codes1 - mu1) * (codes2 - mu2))
        ...
        # Convert similarity to distance (SSIM ranges from -1 to 1, with 1 being most similar)
        distance = (1 - ssim) / 2  # Normalize to [0,1] range
